...
| --- | --- | --- |
| `profileComplete` | `true` exactly when `contactPhone` or `contactEmail` is non-empty. Always written via `computeProfileComplete`. | "Incomplete Profiles" must be an indexed query (`status` + `profileComplete` + `createdAt`), not a scan of the whole collection. The converter recomputes it on read as well, so a profile edited through the Firebase console cannot leave a stale flag behind. |
| `searchName` | Lowercased, single-spaced `"first last"`, written via `buildSearchName` — then widened by `withPinyin` when it holds Chinese: `蔡秉洲` is stored as `蔡秉洲 caibingzhou cbz tsaibingzhou`. | Firestore has no substring search. This is the key the roster's search bar matches against, without loading a secondary index. Recomputed on read if missing. Stored raw apart from case and spacing: accents, punctuation and typos are all folded at match time by `createSearchMatcher`, so the matching rules can change without a migration. The pinyin is there because the lobby kiosk's keyboard has no IME, so a Chinese name is otherwise unreachable by typing; the dictionary that produces it is close to a megabyte and lives only in the functions package (`functions/src/names/pinyin.ts`). Every server write path adds it, `onStudentNamed` catches what the client writes for itself, and `npm run pinyin:backfill` catches up rows nothing has touched. The token order is a contract: the canonical romanization comes first, and `nameSortKey` reads it to file the child under the letter a reader would look for. |
...
**Three moments read that clock, because a tablet is not a page that runs uninterrupted.** The
interval is one. The **boot** is the second: a kiosk reloaded lands on the chooser, and now also
drops the dead binding from localStorage there rather than a minute later, so a tablet opened and
closed inside that minute stops carrying last week's gathering to the next boot. The **wake** is the
third, and it is the one an installed app actually meets: switched away from, or locked with the
browser behind it, a PWA comes back days later to *the same page* — same React state, same binding,
timers frozen by the platform the whole time and no reload anywhere in the journey. So
`visibilitychange` and `pageshow` run the same sweep, and the volunteer opening the app finds the
chooser rather than a gathering that finished last week.
...
| `typedChildren` | `{ firstName, lastName, grade }[]` \| absent | The children exactly as the family typed them, written **once** on the first correction and never overwritten — the point is what the family wrote, not what the last reviewer saw. Absent until somebody corrects something, which is the ordinary case. See [review-corrections.md](review-corrections.md). |
| `typedGuardianName` | `{ firstName, lastName }` \| absent | The adult's name on the same terms. The **name only**: the number they typed is deliberately never kept, because a mistyped one is a stranger's and holding it for thirty days to caption a correction is exactly the retention the TTL below exists to prevent. |
| `phoneCorrected` | boolean | Whether a reviewer replaced the number the family typed. All a second reviewer needs from a number Tally does not keep. |
| `amendedAt`, `amendedBy` | — | The last correction, and who made it. |
