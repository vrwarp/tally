...
Twenty frames of the six journeys in [the brief](../../../uxr/JOURNEY-profile-sync.md)
and every state in [profile-edits.md](../../profile-edits.md), on both layouts,
[as a single page](../edit-queue.html).
...
```bash
npm run walkthrough:edit-queue          # capture both layouts, then build the page
```
