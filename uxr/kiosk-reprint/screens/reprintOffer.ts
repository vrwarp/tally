...
export type ReprintOffer =
  ...
  | 'spent'
  /** Outside the window, or nowhere a label would come out. The common case. */
  | 'none';
...
export function reprintOffer({
  ...
  if (checkedInAt === undefined) return 'none';
  if (now - checkedInAt > OFFER_WINDOW_MS) return 'none';
