...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .who { flex-shrink: 0; }
  /* Leading that survives a second line. Set solid, the first name a character
     longer than the seed data dropped its descenders into the caps beneath —
     on the one element the parent is here to verify. */
  .who .name { font-size: 3rem; line-height: 1.15; font-weight: 700; color: var(--ink-50); }
  ...
  .family .ask { flex-shrink: 0; padding-bottom: 0.75rem; font-size: 1.25rem; line-height: 1.75rem; color: var(--ink-400); }
  ...
  /* One declaration owns this size. r4 added an element-scoped rule and left
     the more specific one in place, so the restoration was inert in exactly
     the quiet way the class rename had been — the note said 20px and the
     pixels said 18. */
  .whoelse .ask {
  ...
  .screen.r3 {
    ...
    /*
     * The bottom, and only the bottom. A fixed band centred by the flex parent
     * anchored both ends — so the portrait glass scrolled a list it had room to
     * print, spending a third of the page on emptiness and a child on clipping
     * in the same view. Full height with the commit held a constant distance up
     * from the bezel: the button does not move, the clear band above it is
     * untouched, and the top edge is free to rise into the page as the family
     * grows.
     */
    height: 100%; padding-bottom: max(2rem, 18vh);
  }
  /*
   ...
   */
  .more {
    flex-shrink: 0; padding: 0.5rem 0 0; font-size: 1.25rem; line-height: 1.75rem;
    color: var(--brand-300); text-align: left; margin: 0;
  }
...
<body style="width:${tall ? 800 : 1280}px;height:${tall ? 1280 : 800}px">
  ...
  <script>
    /*
     * What the list is hiding, said where it is hidden — and decided by
     * measuring rather than by counting.
     *
     * r4 gated the overflow signal on a hand-written "more than four
     * siblings", which was already off by one against the geometry: the list
     * clips at four, so a five-child family got a silent truncation with no
     * fade, no count and no scrollbar. The same failure mode as the class
     * rename one level up — a rule that is right until the thing it is
     * guessing about changes. A name that wraps, a taller grade line or a
     * platform that reserves a scrollbar gutter all move the threshold.
     *
     * The count is a positive mark rather than an absence: a mask says "more"
     * by taking legibility away, which is a strange way to tell somebody a
     * child they are about to check in exists below the line.
     */
    const list = document.querySelector('.rows');
    const more = document.querySelector('.more');
    if (list && more) {
      /*
       * Reveal the count line before measuring, because it is inside the region
       * and takes its height out of the list. Measuring first and revealing
       * afterwards re-cut the list under its own answer — the quantised edge
       * moved back into a row the moment the line appeared.
       */
      more.textContent = '\u00a0';
      more.hidden = false;

      /*
       * Quantised to the row pitch, so the scroll edge always lands in the 8px
       * between cards. A row is either printed or hidden; half a ticked child
       * at half value is the one state this screen cannot render, because a
       * receded name beside a dimmed tick states "not included" in the column a
       * parent scans precisely for that.
       */
      const rows = [...list.children];
      const pitch = 72;
      const fits = Math.max(1, Math.floor((list.clientHeight + 8) / pitch));
      if (rows.length > fits) list.style.height = fits * pitch - 8 + 'px';

      const unread = rows.length - Math.min(rows.length, fits);
      if (unread > 0) {
        more.textContent = unread === 1 ? '1 more below' : unread + ' more below';
      } else {
        more.hidden = true;
      }
    }
