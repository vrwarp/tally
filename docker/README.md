...
- chromium and webkit, from `mcr.microsoft.com/playwright:v1.62.0-noble`
...
**The Playwright version is pinned twice.** The base image tag in
`docker/e2e/Dockerfile` and `@playwright/test` in `package.json` must match, or
the browsers in the image will not be the ones the test runner expects.
Dependabot watches both.
