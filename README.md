...
- **It cannot import from `src/`, so two modules are copied into it.** `functions/src/generated`
  ...
  `scripts/sync-functions-shared.mjs` — the nightly occurrence sweep has to expand recurrence rules
  exactly the way the app does, and a second hand-written expander would drift. **Edit the originals,
