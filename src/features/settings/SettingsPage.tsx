...
export function SettingsPage() {
  ...
  return (
    <PageFrame>
      ...
      {/*
       * Above `lg` the cards stop being one stacked column.
       *
       * `PageFrame` widens to `max-w-7xl` beside the rail, and a card that
       * takes all of that spends it on line length: the connection cards were
       * setting a two-clause sentence as one ~990px line. The recovered width
       * belongs to columns, so the cards pair up instead — and the whole
       * screen, which is one a leader opens to check one thing and confirm
       * three others, lands above the fold rather than under it.
       *
       * The two that span both columns are the two that already have an
       * internal split of their own: the thresholds beside their preview, and
       * the theme picker beside the sentence that describes it. That leaves
       * Planning Center and Attendees side by side, which is also where they
       * belong — they answer the same question about two backends.
       */}
