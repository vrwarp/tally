...
const GRAMMAR_NOTES: Record<TargetLocale, string> = {
  'es-MX':
    'Spanish needs BOTH plural branches — `one` and `other` — wherever English has them. Where English is genderless about a person, Spanish must not guess: these strings sit beside a student\'s name and the app does not know whether that student is a girl or a boy, so prefer a verb over a participle ("ya llegó", not "registrado") and a genderless noun over an adjective — at the kiosk the child is "su hijo o hija", never "niño". That rule is about people only: a gathering is "reunión cancelada" and a sync chip is "Cancelado", and rewriting those as "que se canceló" is a defect. Address a parent at the lobby kiosk as *usted*; address staff inside the app as *tú*. Spanish runs 20-25% longer than English by default and this app has no room for it, so use the compression a Spanish UI already uses: a noun phrase rather than a sentence ("Kiosco sin impresora", not "Este kiosco no tiene ninguna impresora"), no copula in short lines ("Comando no disponible"), and no article a label can live without ("Guardar cambios", never "Guardar los cambios").',
  'zh-Hans':
    'Chinese needs no plural branches — `{count, plural, other {...}}` alone is correct. Drop the copula and the pronoun wherever the screen supplies them, and let the aspect markers (已, 中, 过) do the work English spends a tense on: a status chip is 已签到, not a clause. Chinese renders about three quarters of the English width, so there is room — do not spend it explaining. A label names its state; the explanation belongs in the tooltip key beside it. The English is deliberately chatty; do not mirror that. The ordinary written register of a mainland app (亦可, 均可, 尚未) is the target, one notch more formal than the English, while any 你 a `context` note asks for stays. Quote a button name with “ ”, never 「 」. "Leader" is 辅导, always; 同工 is the generic team member.',
  'zh-Hant':
    'Chinese needs no plural branches — `{count, plural, other {...}}` alone is correct. Drop the copula and the pronoun wherever the screen supplies them, and let the aspect markers (已, 中, 過) do the work English spends a tense on: a status chip is 已簽到, not a clause. Chinese renders about three quarters of the English width, so there is room — do not spend it explaining. A label names its state; the explanation belongs in the tooltip key beside it. The English is deliberately chatty; do not mirror that. The ordinary written register of a Taiwan app (皆可, 尚未, 毋需) is the target, one notch more formal than the English, while any 你 a `context` note asks for stays. Quote a button name with 「 」, never “ ”. "Leader" is 輔導, always; 同工 is the generic team member.',
};
