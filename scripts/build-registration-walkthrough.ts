...
const flows: { flow: string; shots: Shot[] }[] = [];
for (const shot of shots) {
  const last = flows[flows.length - 1];
  if (last && last.flow === shot.flow) last.shots.push(shot);
  else flows.push({ flow: shot.flow, shots: [shot] });
}
...
for (const group of flows) {
  ...
  for (const shot of group.shots) {
    index += 1;
    items.push(`      <figure class="frame">
        <div class="frame__meta">
          <span class="frame__num">${String(index).padStart(2, '0')}</span>
          <span class="frame__state">${escapeHtml(shot.state)}</span>
        </div>
        <h3 class="frame__title">${escapeHtml(shot.title)}</h3>
        <figcaption class="frame__caption">${escapeHtml(shot.caption)}</figcaption>
        <div class="frame__shot">
          <img src="${await dataUri(shot.file)}" alt="${escapeHtml(shot.title)}" loading="lazy" width="1280" height="720" />
        </div>
      </figure>`);
  }
