...
/**
 * The smallest thing that behaves like the Firestore surface the sync uses:
 * document get/set/update, one collection read, and batched writes. It also
 * records every committed write so a test can assert that an unchanged student
 * produced *no* write at all — which is the property that keeps a sync from
 * waking every counselor's phone.
 *
